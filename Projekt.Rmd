...
Aby jednak tego dokonać zastosujemy dekompozycję SVD badanej macierzy standaryzowanych reszt pearsonowskich $E$. Metoda ta stanowi uogólnienie diagonalizacji na macierze niekwadratowe takie, jak badana macierz $E$ wymiarów $41\times 12$. Dekompozycja ta polega na znajdowaniu wartości osobliwych, czyli dodatnich pierwiastków wartości własnych macierzy powstałej z pomnożenia macierzy $E$ przez jej transpozycję w dowolnej kolejności (niezerowe wartości własne macierzy $E\cdot E^T$ i $E^T\cdot E$ są równe). Wektory osobliwe, czyli wektory własne pochodzące od poszczególnych wartości własnych macierzy $E\cdot E^T$ wyznaczają bazy przestrzeni operatorów liniowych najlepiej aproksymujących operator liniowy o macierzy $E$, dzięki czemu możemy przy ich użyciu ograniczyć się do rozważania tylko kilku wymiarów tracąc jak najmniej informacji o całych danych. Skorzystamy do tego celu z dostępnej w R funkcji svd:
...
```{r, echo = FALSE}
wyniki_2023_okr <- wyniki_2023_okr[-c(32)]
str(wyniki_2023_okr)
dane3 <- dane2[-c(32), ]
str(dane3)
```
...
Na podstawie tych wykresów jesteśmy w stanie odczytać, iż partia PJJ (Polska Jest Jedyna) zdaje się znacząco odstawać od reszty elementów wykresu oraz okrąg wyborczy 14 (w skład którego wchdzą ) wykazuje szczególne poparcie dla Lewicy. W dalszej kolejności pominiemy więc partię PJJ.

# Końcowa analiza korespondencji z podziałem na okręgi wyborcze
...
Na podstawie tych wykresów jesteśmy w stanie odczytać, iż partia PJJ (Polska Jest Jedyna) zdaje się znacząco odstawać od reszty elementów wykresu oraz okrąg wyborczy 14 (w województwie śląskim) wykazuje szczególne poparcie dla Lewicy, okręgi 19(Warszawa) i 39(Poznań) wykazują poparcie przede wszystkim dla Koalicji Obywatelskiej. Podobnie możemy odczytać resztę zależności.
